...
def test(config, 
    ...
    # Extract features
    # 这里最重要的就是第一个qf，feature，后面用这个feature获得distmat就是特征举证？
    # 其他的内容单纯就是从dataloader中提取出来的，不足为奇
    ...
    # 既然如此，distamat中如何与pid进行比较的内容，就在evaluate中了
    ...
    cmc, mAP = evaluate_with_clothes(distmat, 
                                    q_pids, g_pids, 
                                    q_camids, g_camids, 
                                    q_clothes_ids, g_clothes_ids,
                                    mode='SC')
    logger.info("Results ---------------------------------------------------")
    logger.info(
        'top1:{:.1%} top5:{:.1%} top10:{:.1%} top20:{:.1%} mAP:{:.1%}'.format(cmc[0], cmc[4], cmc[9], cmc[19], mAP))
