...
from tools.utils import AverageMeter # 用于更新和存储值
...
def train_model(config, epoch, 
    ...
    for batch_idx, (parsing_results, imgs, imgs_b, pids, clothes_ids) in enumerate(trainloader):
        ...
        # 这就是所谓的训练了一个head-enhanced的模块
        attention_maps = attention(features)
        parsing_results = Resize(parsing_results) # 24*12
        ...
        # 拉近三个E之间的距离
        vc_loss = mse_loss(features_att, features_pool) \
                    + mse_loss(features_att, head_features_pool) \
        ...
        # 记录预测了哪些数据，后面用作准确率的指标做计算
        _, preds = torch.max(features_out.data, 1)

        # Please adjust the weight of loss functions according to different datasets
        loss = part_loss + vc_loss \
                + pair_loss_f + pair_loss_h + pair_loss_b \
                + cla_loss_f + cla_loss_h + cla_loss_b
