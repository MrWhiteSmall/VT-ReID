...
def denormalize(img, mean=0, std=1):
    # 反归一化处理: (img * std) + mean
    return (img * std) + mean
# @torch.no_grad()
def extract_img_feature_by_vton(model, dataloader,vton_model=None,get_vton_by_model=None,one_batch=False,have_batch=None):
    ...
    vton_imgs = None
    try:
        cloth_img,masked_vton_img,mask,model_img = get_vton_by_model(cloth_path,img_paths[0],save=True,no_grad=False)
        images,prompt_loss = vton_model(
            model_type='hd',
            category='upperbody',
            ...
            image_scale=2.0,
            seed=-1,
            no_grad = False, # False : 需要grad True : 不需要grad
        )
        test_imgs = images
    ...
    if test_imgs is not None:
        ...
        print('normalized vton img tensor',normalized_imgs.shape,normalized_imgs.requires_grad)
    ...
    ######################################################################################
    # 收集一下，原图 + 衣服 + vton-img
    # origin_img   <class 'torch.Tensor'> torch.Size([1, 3, 384, 192])
    # print(f'origin_img   {type(imgs_ori)} {imgs_ori.shape}')
    # target_cloth <class 'PIL.Image.Image'> (384, 192, 3)
    # print(f'target_cloth {type(cloth_img)} {np.array(cloth_img).shape}')
    # vton_img    <class 'torch.Tensor'> torch.Size([4, 3, 384, 192])
    # print(f'vton_img    {type(imgs)} {imgs.shape}')
    origin_img = imgs_ori.clone().permute(0,2,3,1).cpu().detach().numpy()
    target_cloth = np.array(cloth_img) # 可以直接保存
    vton_img = imgs.clone().permute(0,2,3,1).cpu().detach().numpy()
    
    # 反归一化 origin_img 和 vton_imgs
    origin_img = denormalize(origin_img, mean=0, std=1)  # 修改为适当的mean和std
    vton_img = denormalize(vton_img, mean=0, std=1)    # 修改为适当的mean和std
    ...
    # else:
    #     save_name = 'second.jpg'
    save_path = f'/data/lsj/OOTDiffusion/run/images_output/{save_name}'
...
def main_vcc(get_vton_model = None,get_vton_by_model=None):
    ...
    ##################### 3 extract_img_feature提取特征 #############################
    vton_model = get_vton_model(0)
    optimizer = optim.Adam(vton_model.learnable_model.parameters(), lr=1e-3)
    ...
    qfs, q_pids, q_camids, q_clothes_ids, features_ori,sim_prompt_loss,sim_cosine_loss,have_batch =  \
                                            extract_img_feature_by_vton( model, queryloader,
