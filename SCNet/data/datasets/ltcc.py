...
class LTCC(object):
    ...
    """
    dataset_dir = 'LTCC-res'
    def __init__(self, root='data', **kwargs):
        # 原版
        self.dataset_dir = osp.join(root, self.dataset_dir)
        # 加了vt后
        # self.dataset_dir = osp.join(root, self.dataset_dir,'vt')
    ...
    def _process_dir_test(self, query_path, gallery_path):
        # gallery 即 test
        # query_vt  006_1_c9_000151_02763_00.jpg.png
        # query     006_1_c9_000151.png
        # query_vt.startwith( query.split('.')[0] )
        # query_vt_path = '/data1/lsj/ccdatasets/LTCC_ReID/query_vt_res'
        # gallery_vt_path = '/data1/lsj/ccdatasets/LTCC_ReID/test_vt_res'
        query_img_paths = glob.glob(osp.join(query_path, '*.png'))
        gallery_img_paths = glob.glob(osp.join(gallery_path, '*.png'))
        # query_vt_img_paths = glob.glob(osp.join(query_vt_path, '*.png'))
        # gallery_vt_img_paths = glob.glob(osp.join(gallery_vt_path, '*.png'))
        query_img_paths.sort()
        gallery_img_paths.sort()
        # query_vt_img_paths.sort()
        # gallery_vt_img_paths.sort()
        pattern1 = re.compile(r'(\d+)_(\d+)_c(\d+)')
        pattern2 = re.compile(r'(\w+)_c')
